...
export default {
  ...
  fields: [
    {
      ...
      name: 'size',
      type: 'number',
      description: 'Set the default number of column spaces available for this grid',
      options: {
        list: [
          {title: '1', value: 1},
          {title: '2', value: 2},
  ...
  preview: {
    ...
    prepare({columns}) {
      ...
      const image = (columns[0].blocks[0].content || []).find((block) => block._type === 'photo')

      return {
        title: `${columns.length} Column${columns.length > 1 ? 's' : ''}`,
        subtitle: name,
        media: image ?? ThLargeIcon,
      }
    },
  },
}
